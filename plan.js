...
var PlanGroup = Class({
	...
	/**
	 * Executes all plans that we can afford, ordered by priority, and returns
	 * the highest-priority plan we couldn't afford (or null if none).
	 */
	executePlans : function(gameState) {
		// Ignore impossible plans
		var plans = this.plans.filter(function(p) {
			return p.plan.canExecute(gameState);
		});
